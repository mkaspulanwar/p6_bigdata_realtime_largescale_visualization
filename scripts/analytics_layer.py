...
from pyspark.sql.functions import sum as _sum, avg, desc, round as spark_round
import os
import time
...
total_revenue = df_clean.agg(
    spark_round(_sum("total_amount"), 0).cast("long").alias("total_revenue")
)
...
category_revenue = df_clean.groupBy("category") \
    .agg(spark_round(_sum("total_amount"), 0).cast("long").alias("category_revenue")) \
...
avg_transaction = df_clean.groupBy("customer_id") \
    .agg(spark_round(avg("total_amount"), 0).cast("long").alias("avg_transaction_value"))
